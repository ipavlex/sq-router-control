...
(function () {
  ...
  function renderInputs(inputs) {
    ...
    for (const r of merged) {
      ...
      tr.innerHTML =
        `<td class="ch-cell">${utils.escapeHtml(r.destLabel)}</td>` +
        `<td class="name-cell">${utils.escapeHtml(r.name || "—")}</td>` +
        `<td class="src-cell">${utils.escapeHtml(r.sourceLabel)}</td>` +
        `<td>${utils.escapeHtml(inLabel)}</td>`;
      frag.appendChild(tr);
    }
  ...
  function defaultSaveName() {
    return state.currentSceneName ? `${utils.todayStr()} ${state.currentSceneName}` : utils.todayStr();
  }
  ...
  function flashActivePatching() {
    utils.flashTitle(els.activePatchingTitle);
  }

  /** Make the "Input Patching" title blink green once. */
  function flashInputPatching() {
    utils.flashTitle(els.inputPatchingTitle);
  }
   ...
   */
  function onRoutingSnapshot(snap) {
    ...
    if (pairsKey !== JSON.stringify(state.stereoPairs)) {
      ...
      SQ.monitor.buildChannelButtons();
