...
### Добавлено
- В списке INPUT PATCHING у каждой строки появился чекбокс для выбора каналов,
  которые будут отправлены на пульт при Upload. По умолчанию не установлен;
  при любом изменении параметров строки (источник/вход) чекбокс ставится
  автоматически.
- Вместо текстового заголовка колонки — чекбокс «выбрать все»: отмечает или
  снимает все строки разом, показывает промежуточное состояние при частичном
  выборе.
- Upload отправляет на пульт только отмеченные каналы.

### Изменено
- Состояние чекбоксов хранится в наборах A/B и сохраняется при переключении
  списков и после отправки на пульт. Строка отмечается вручную или при
  изменении её параметров — обновления Active Patching на выделение не влияют.
- При загрузке сохранённого роутинга строки, отличающиеся от текущего роутинга
  пульта, отмечаются автоматически, чтобы Upload был сразу применим.
