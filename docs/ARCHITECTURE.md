...
| [`VERIFY-STEREO-MODES.md`](VERIFY-STEREO-MODES.md) | Как подтвердить режим моно/стерео миксов и матриц (дамп + diff) |
...
- не декодированы некоторые метро-пакеты (раскладка mix/Main LR подтверждена
  лишь частично);
- режим стерео/моно миксов читается из ParamData (`+331` блока канала,
  см. `stereo-links.ts`); режим матриц (стерео/разбито в моно) пока
  определяется по link-региону не полностью;
